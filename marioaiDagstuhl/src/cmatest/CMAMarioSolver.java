...
public class CMAMarioSolver {
	...
	public static final int EVALS = 10000;

	
    public static void main(String[] args) throws IOException {
        Settings.setPythonProgram();
        for (MarioEvalFunction.fitMetric activeFitMetric : MarioEvalFunction.fitMetric.values()) {
            int Noflevels = 2;
            double[][] bestX = new double[Noflevels][32];
            double[] bestY = new double[Noflevels];
            MarioEvalFunction marioEvalFunction = new MarioEvalFunction(activeFitMetric);
            ...
            try (PrintWriter levelsFile = new PrintWriter( new FileWriter("levels_output.json " + activeFitMetric , false))) {
                levelsFile.print("[");
                for(int i=0;i<Noflevels; i++){

                    String levelString = marioEvalFunction.jsonFromLatentVector(bestX[i]);
                    levelsFile.println(levelString);
                    if(i != Noflevels-1) {
                        levelsFile.print(",");
                    }
                }
            ...
            try (PrintWriter latentVectorsFile = new PrintWriter(new FileWriter("latent_vectors_output.json " + activeFitMetric, false))) {
                for (int i = 0; i < Noflevels; i++) {
                    latentVectorsFile.println(Arrays.toString(bestX[i]));
                    System.out.println(Arrays.toString(bestX[i]));
                }
                latentVectorsFile.println(Arrays.toString(bestY));
            }
    ...
    public double[] run(PrintWriter print_line) {
        ...
        while (cma.stopConditions.getNumber() == 0) {
            ...
            for (int i = 0; i < pop.length; ++i) {    // for each candidate solution i
                System.out.println(i + " out of " + pop.length);
                ...
                System.out.println("Fittness: " + fitness[i]);
                print_line.println(Arrays.toString(pop[i])+ " : " + fitness[i]);
                System.out.println("Objective function evals:" + cma.getCountEval());
            }
